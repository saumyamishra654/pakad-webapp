...
Zeelaf,S-G-m-P-d-S,S-d-P-m-G-S`,R={S:0,r:1,R:2,g:3,G:4,m:5,M:6,P:7,d:8,D:9,n:10,N:11};function N(n){const a=new Array(12).fill(0);a[0]=1;for(const S of n)Object.prototype.hasOwnProperty.call(R,S)&&(a[R[S]]=1);return a}function h(n){const a=n.split(/\r?\n/).filter(Boolean),S=[];for(let i=1;i<a.length;i++){const[r,D,m]=a[i].split(",");if(!r||!D||!m)continue;const P=N(D),G=N(m),t=P.map((e,M)=>e||G[M]?1:0);S.push({name:r.trim(),notePattern:t,aarohPattern:P,avrohPattern:G,type:"hindustani"})}return S}const c=h(d),g=`number,name,swaras
...
72,Rasikapriya,S R₃ G₃ M₂ P D₃ N₃ S`;function s(n){const a=new Array(12).fill(0),S=new Array(16).fill(!1),i=n.trim().split(/\s+/),r=D=>{D>=0&&(a[(D%12+12)%12]=1)};for(const D of i){const m=D.replace(/₁/g,"1").replace(/₂/g,"2").replace(/₃/g,"3");if(m==="S"){r(0),S[0]=!0;continue}if(m==="P"){r(7),S[9]=!0;continue}if(m==="M1"){r(5),S[7]=!0;continue}if(m==="M2"){r(6),S[8]=!0;continue}if(m.startsWith("R")){const P=m.slice(1);P==="1"?(r(1),S[1]=!0):P==="2"?(r(2),S[2]=!0):P==="3"&&(r(3),S[4]=!0);continue}if(m.startsWith("G")){const P=m.slice(1);P==="1"?(r(2),S[3]=!0):P==="2"?(r(3),S[5]=!0):P==="3"&&(r(4),S[6]=!0);continue}if(m.startsWith("D")){const P=m.slice(1);P==="1"?(r(8),S[10]=!0):P==="2"?(r(9),S[11]=!0):P==="3"&&(r(10),S[13]=!0);continue}if(m.startsWith("N")){const P=m.slice(1);P==="1"?(r(9),S[12]=!0):P==="2"?(r(10),S[14]=!0):P==="3"&&(r(11),S[15]=!0);continue}}return a[0]=1,S[0]=!0,{notePattern:a,carnaticSwaras:S}}function o(n){const a=n.split(/\r?\n/).filter(Boolean),S=[];for(let i=1;i<a.length;i++){const r=a[i].split(",");if(r.length<3)continue;const D=parseInt(r[0]),m=r[1]?.trim(),P=r.slice(2).join(",").trim();if(!m||!P)continue;const G=s(P);S.push({number:D,name:m,notePattern:G.notePattern,carnaticSwaras:G.carnaticSwaras,aarohPattern:G.notePattern,avrohPattern:G.notePattern,type:"carnatic"})}return S}const p=o(g),u=["Sa","Re♭","Re","Ga♭","Ga","Ma","Ma♯","Pa","Dha♭","Dha","Ni♭","Ni"],l=["S","R1","R2/G1","R3/G2","G3","M1","M2","P","D1","D2/N1","D3/N2","N3"],A=["C","C#","D","D#","E","F","F#","G","G#","A","A#","B"],v=["C","Db","D","Eb","E","F","Gb","G","Ab","A","Bb","B"],f=[3,4,5],y={3:"_m12",4:"",5:"_p12"};function k(n){return n?l:u}function B(n,a=4){const i=(a-4)*12+(n-9);return 440*Math.pow(2,i/12)}function K(n,a){return((n+a)%12+12)%12}function T(n,a){const S=new Array(12).fill(0);for(let i=0;i<12;i++)S[(i+a)%12]=n[i];return S}function w(n){return n.map(a=>a?"1":"0").join("")}function b(n){return n.reduce((a,S)=>a+(S?1:0),0)}function C(n){return n.map((a,S)=>a?S:null).filter(a=>a!==null)}function j(n){const a=new Array(12).fill(0);for(const S of n)a[(S%12+12)%12]=1;return a}function H(n,a){for(let S=0;S<12;S++)if(n[S]&&!a[S])return!1;return!0}function _(n,a){return n.reduce((S,i,r)=>S+(i!==(a[r]||0)?1:0),0)}export{f as A,c as H,p as M,y as N,v as S,A as W,C as a,w as b,b as c,K as d,B as e,k as g,_ as h,H as i,j as p,T as r};
